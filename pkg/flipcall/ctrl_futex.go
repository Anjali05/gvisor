...
func (ep *Endpoint) ctrlConnect() error {
	...
	w := ep.NewWriter()
	if err := json.NewEncoder(w).Encode(handshakeRequest{}); err != nil {
		return fmt.Errorf("error writing handshake request: %v", err)
	}
	*ep.dataLen() = w.Len()
	...
	if respLen > ep.dataCap {
		return fmt.Errorf("invalid handshake response length %d (maximum %d)", respLen, ep.dataCap)
	}
...
func (ep *Endpoint) ctrlWaitFirst() error {
	...
	w := ep.NewWriter()
	if err := json.NewEncoder(w).Encode(handshakeResponse{}); err != nil {
